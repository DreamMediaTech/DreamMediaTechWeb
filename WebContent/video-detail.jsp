                ...
                <div class="row">
                    ...
                    <div class="span9">
                        <div class="content">
                           ...
                           <tr>
                           <td  colspan="2">
                           <video src="${video_detail.getvAddress()}" controls="controls" weight="400px" height="225px"></video>
                           </td>
                           </tr>
                           <tr>
                           <td><label>视频标题:${video_detail.getvTitle()}</label></td>
                           <td><label>视频状态:
                           <c:if test="${video_detail.getvState()==1}">审核通过</c:if>
                           <c:if test="${video_detail.getvState()==0}">等待审核</c:if>
                           <c:if test="${video_detail.getvState()==-1}">内容违规</c:if>
                           <c:if test="${video_detail.getvState()==2}">正在转码</c:if>
                           </label></td>
                           </tr>
                           <tr>
                           <td><label>视频作者:${video_detail.getAuthor().getuNickName()}</label></td>
                           <td><label>作者收益:${video_detail.getvProfit()}</label></td>
                           </tr>
                           <tr>
                           <td><label>视频播放量:${video_detail.getvNum()}</label></td>
                           <td><label>视频价格:${video_detail.getvPrice()}</label></td>
                           </tr>
                           <tr>
                           <td colspan="2"><label>视频简介:</label></td>
                           </tr>
                           <tr >
                           <td colspan="2"><textarea>${video_detail.getvIntroduce()}</textarea></td>
                           </tr>
                           <c:if test="${video_detail.getvState()==0}">
                           <tr >
                           <td><a href="${pageContext.request.contextPath}/videoController/auditVideo.action?vid=${video_detail.getvId()}&state=2" class="btn btn-primary">审核通过</a></td>
                           <td><a href="${pageContext.request.contextPath}/videoController/auditVideo.action?vid=${video_detail.getvId()}&state=-1" class="btn btn-primary">审核不通过</a></td>
                           </tr>
                           </c:if>
                           </table>
                                </div>

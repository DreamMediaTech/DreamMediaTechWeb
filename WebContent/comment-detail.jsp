                    ...
                    <div class="span9">
                        <div class="content">
                            ...
                            <div class="module-body">
                                <h2 class="text-center" align="center">${article.getaTitle()}</h2>
                             
                                <hr/>
                                <section class="docs">
                                    <div id="notive_info" align="left">
                                    <table>
                                    <tr>
                                    <td>评论人：${Comment_detail.getcUser().getuNickName()}</td>
                                    <td>评论时间：${Comment_detail.getcTime()}</td>
                                    </tr>
                                    <tr>
                                    <td>评论视频：${Comment_detail.getVideo().getvTitle()}</td>
                                    <td>评论状态：
                                    <c:if test="${Comment_detail.getcState()==0}">等待审核</c:if>
                                    <c:if test="${Comment_detail.getcState()>0}">审核通过</c:if>
                                    <c:if test="${Comment_detail.getcState()<0}">内容违规</c:if>
                                    </td>
                                    </tr>
                                    <tr>
                                    <td>评论内容：</td>
                                    ...
                                    <tr>
                                    <td colspan="2">${Comment_detail.getcContent()}</td>
                                    </tr>
                                    <c:if test="${Comment_detail.getcState()==0}">
                                    <tr >
                                    <td><a href="${pageContext.request.contextPath}/videoController/auditComment.action?cid=${Comment_detail.getcId()}&state=1&vid=${Comment_detail.getVideo().getvId()}" class="btn btn-primary">审核通过</a></td>
                                    <td><a href="${pageContext.request.contextPath}/videoController/auditComment.action?cid=${Comment_detail.getcId()}&state=-1&vid=${Comment_detail.getVideo().getvId()}" class="btn btn-primary">审核不通过</a></td>
                                    </tr>
                                    </c:if>
                                    </table>
                                </section>

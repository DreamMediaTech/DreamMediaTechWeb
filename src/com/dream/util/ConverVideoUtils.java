...
public class ConverVideoUtils {  
     ...
     * @return 
     */  
    public boolean beginConver(String targetExtension, boolean isDelSourseFile,int encodetype) {  
        ...
        if (process(targetExtension,isDelSourseFile,encodetype)) {  
...
//      commend.add("320x240");  
     ...
     * @return 
     */  
    private boolean process(String targetExtension, boolean isDelSourseFile,int encodetype) {  
        int type = checkContentType();  
        boolean status = false;  
        if (type == 0) {  
            //如果type为0用ffmpeg直接转换  
            status = processVideoFormat(sourceVideoPath,targetExtension, isDelSourseFile,encodetype);  
        } else if (type == 1) {  
            ...
            if (avifilepath == null){  
                ...
                status = processVideoFormat(avifilepath,targetExtension, isDelSourseFile,encodetype);  
            }  
        }  
        return status;  
    }  
     ...
     * @return 
     */  
    private boolean processVideoFormat(String oldfilepath, String targetExtension, boolean isDelSourceFile,int type) {  
        if (!checkfile(sourceVideoPath)) {  
		...
		case Contants.type_sd_code:
	        ...
	        commend.add("480x360");
	        commend.add("-y");  
	        commend.add(targetfolder + filerealname+"_sd" + targetExtension);  
			break;
		case Contants.type_hd_code:
	        commend.add(ffmpegpath);  
	        commend.add("-i");  
	        commend.add(oldfilepath);  
	        commend.add("-qscale");     //指定转换的质量    
	        ...
	        commend.add("640x480");
	        commend.add("-y");
	        commend.add(targetfolder + filerealname+"_hd" + targetExtension);  
	        break;
		case Contants.type_ud_code:
	        commend.add(ffmpegpath);  
	        commend.add("-i");  
	        commend.add(oldfilepath);  
	        commend.add("-qscale");     //指定转换的质量    
	        ...
	        commend.add("-y");
	        commend.add(targetfolder + filerealname+"_ud" + targetExtension);  
	        break;
		}
        try {  
            ProcessBuilder builder = new ProcessBuilder();  
